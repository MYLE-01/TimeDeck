    ...
    .then(data => {
      ...
      modal.innerHTML = `
        <div class="bg-white rounded-xl shadow-xl p-6 w-[360px]">
          ...
          <button class="mt-4 bg-blue-600 text-white px-4 py-2 rounded" onclick="saveEditedHours('${empId}', '${date}')">💾 Save</button>
          <button class="mt-2 ml-2 bg-gray-400 text-white px-4 py-2 rounded" onclick="this.closest('.fixed')?.remove()">✖️ Cancel</button>
        </div>
      `;
  ...
  modal.innerHTML = `
    <div class="bg-white p-6 rounded-xl shadow-xl w-[360px]">
      ...
      <div class="flex justify-end space-x-2">
        ...
        <button class="bg-blue-600 text-white px-4 py-2 rounded" onclick="submitExtraEntry('${empId}', '${date}')">💾 Save</button>
      </div>
    </div>
  `;
  document.body.appendChild(modal);
          ...
          return `
            <tr class="${rowClass}" data-id="${entry.ID}" data-date="${entry.date}" data-shift_name="${entry.shift_name || ''}" data-shift="${entry.shift_type || ''}" data-type="${entry.type_shift || ''}">
              ...
              <td class="border px-2 py-1 text-center">
                <button class="text-red-600 hover:text-red-800 deleteRowBtn">🗑</button>
              </td>
            </tr>
          `;
        }).join("");
              ...
              <div class="flex-1 flex justify-center items-end">
                <div class="text-center">
                  <div class="text-sm text-gray-600">
                    <button id="addExtraEntryBtn" class="w-full px-4 py-2 text-sm rounded shadow-sm hover:shadow-md transition text-white bg-green-600 hover:bg-green-700">➕ Add/Edit Entry Shift</button>
                  </div>
                </div>
              </div>
              <div class="flex-1 flex justify-center items-end">
                <div class="text-center">
                  <div class="text-sm text-gray-600">
                    <button class="bg-gray-500 hover:bg-gray-700 text-white px-4 py-2 rounded" onclick="this.closest('.fixed')?.remove()">❌ Close</button>
                  </div>
                </div>
              </div>
            </div>
...
function showTimeOffModal(empId, config, dateInput = "", shiftTypeInput = "", roster = [], onClose = null) {
  ...
  modal.innerHTML = `
    <div class="bg-white p-6 rounded-xl shadow-xl w-[550px] max-h-[90vh] overflow-y-auto scroll-smooth">
      <h2 class="text-lg font-bold mb-4">🛫 Adjust Shift Pattern for ${config.emp_name}</h2>
      ...
      <div class="flex justify-between items-center mt-4">
        <button id="addRowBtn" class="flex items-center gap-1 px-3 py-1.5 text-sm rounded-lg bg-red-100 text-blue-700 hover:bg-red-200 transition">
          <span class="text-lg">➕</span><span>Add Row</span>
        </button>
        <div class="space-x-1">

          <button class="bg-blue-600 text-white px-3 py-2 rounded" id="saveTimeOffBtn">💾 Save</button>
      ...
      <div id="qrFoldout" class="flex flex-col items-center transition-all duration-300 overflow-hidden max-h-0 opacity-0">
        ...
        <div id="qrSpinner" class="hidden mb-2 text-gray-600 text-sm">⏳ Generating QR...</div>
...
toggleBtn.addEventListener("click", async () => {
  ...
  if (isOpen) {
    ...
    try {
      ...
      if (!shift_type) {
        alert("⚠️ Please select Day or Night before generating QR.");
        return;
      }


      console.log("🔍 Generating QR for:", { dateInputLive, shift_type, empId });
      ...
      rows.forEach(row => {
        ...
        // ✅ Only push if mins > 0 OR it's a CASH request with mins = 0
      ...
      if (entries.length) {
        flashButton(saveBtn, "bg-red-300")
      ...
      try {
        ...
        if (result.status === "ok") {
          ...
          console.log("✅ QR generated at:", preview.src);
          ...
          showToast("⚠️ QR generation failed: " + result.message);
        }
      } catch (err) {
...
// Double-click on Notes -> autofill "Cover for <selected shift>" if empty
  ...
  const addPoint3Btn = document.getElementById("addpoint3");


  if (addPoint3Btn) {
    // Left-click → +0.3 hrs (18 mins) or +1 hr (60 mins) with Shift
    addPoint3Btn.addEventListener("click", (e) => {
      if (e.shiftKey) {
        addMinutesToLastRow(60);
        flashButton(addPoint3Btn, "bg-green-300"); // green for +1 hr
      } else {
        addMinutesToLastRow(18);
        flashButton(addPoint3Btn, "bg-blue-300"); // blue for +0.3
      }
    });

    // Right-click → -0.3 hrs (18 mins)
    addPoint3Btn.addEventListener("contextmenu", (e) => {
      e.preventDefault();
      addMinutesToLastRow(-18);
      flashButton(addPoint3Btn, "bg-red-300"); // red for -0.3
    });
  }

  // 🔄 Flash helper
  function flashButton(btn, colorClass) {
    btn.classList.add(colorClass, "transition", "duration-300");
    setTimeout(() => btn.classList.remove(colorClass), 300);
  }
  ...
  saveBtn.addEventListener("click", () => {
    ...
    fetch("/save_time_off", {
      ...
      .then(result => {
        if (result.status === "ok") {
          modal.remove();
          showToast("✅ Roster entries saved");
          if (onClose) onClose();
        } else {
...
function addTimeOffRow(empId, configOverride, defaultData = {}) {
  ...
  //notesAutofillBtn.innerHTML = "✨ Autofill";
...
function showModal(date, empId, config_entitlements, selected_value) {
  ...
  const modalContent = `
    <div class="bg-white p-6 rounded-xl shadow-xl w-[360px]">
      ...
      <div class="flex justify-end space-x-2">
        <button class="bg-gray-500 text-white px-4 py-2 rounded" onclick="this.closest('.fixed')?.remove()">Cancel</button>
        <button id="saveButton" class="bg-blue-600 text-white px-4 py-2 rounded" onclick="submitExtraEntry('${empId}', '${date}')">💾 Save</button>
      </div>
    </div>
  `;
